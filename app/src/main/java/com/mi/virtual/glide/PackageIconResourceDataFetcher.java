...
public class PackageIconResourceDataFetcher implements DataFetcher<InputStream> {
    ...
    private PackageInfo getPackageInfo() throws PackageManager.NameNotFoundException {
        if (packageModel.startsWith(PackageIconResourceLoader.DATA_PACKAGE_PREFIX)) {
            return context.getPackageManager().getPackageInfo(getPackageTrueModel(PackageIconResourceLoader.DATA_PACKAGE_PREFIX), 0);
        } else if (packageModel.startsWith(PackageIconResourceLoader.DATA_PACKAGE_FILE_PATH_PREFIX)) {
            return context.getPackageManager().getPackageArchiveInfo(getPackageTrueModel(PackageIconResourceLoader.DATA_PACKAGE_FILE_PATH_PREFIX), 0);
        }
        return null;
    }
