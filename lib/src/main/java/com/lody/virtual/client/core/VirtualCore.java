    ...
    public void startup(Context context) {
        if (!isStartUp) {
            if (Looper.myLooper() != Looper.getMainLooper()) {
                Log.d("kkkk", "==VirtualCore.startup() must called in main thread.==" + System.currentTimeMillis() / 1000);
                throw new IllegalStateException("VirtualCore.startup() must called in main thread.");
            }
            try {
                Reflection.unseal(context);
            } catch (Exception e) {
                Log.d("kkkk", "==Reflection.unseal==eee==" + System.currentTimeMillis() / 1000);
            }
...
//                throwable.printStackTrace();
            ...
            Log.d("kkkk", "==startup==end==" + System.currentTimeMillis() / 1000);
        }
    }
    ...
    public boolean isEngineLaunched() {
        ...
        for (ActivityManager.RunningAppProcessInfo info : am.getRunningAppProcesses()) {
            Log.d("kkkk", engineProcessName + "==engineProcessName==000==" + System.currentTimeMillis() / 1000);
