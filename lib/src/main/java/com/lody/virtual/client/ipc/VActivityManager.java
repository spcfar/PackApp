...
public class VActivityManager {
    ...
    public IActivityManager getService() {
        if (mRemote == null || (!mRemote.asBinder().pingBinder() && !VirtualCore.get().isVAppProcess())) {
            synchronized (VActivityManager.class) {
                Log.d("kkkk", "==synchronized==000==" + System.currentTimeMillis() / 1000);
                final Object remote = getRemoteInterface();
                mRemote = LocalProxyUtils.genProxy(IActivityManager.class, remote);
            }
        }
        Log.d("oooo", "==mRemote==000==" + System.currentTimeMillis() / 1000);
        return mRemote;
    }
    ...
    public int startActivity(Intent intent, ActivityInfo info, IBinder resultTo, Bundle options, String resultWho, int requestCode, int userId) {
        try {
            if (intent != null) {
                Log.d("kkkk", "==intent==" + intent + "==resultWho==" + resultWho + "==requestCode==" + requestCode + "==userId==" + userId);
            }
            Log.d("kkkk", "==getService().startActivity==000==" + System.currentTimeMillis() / 1000);
            IActivityManager iActivityManager = getService();
            int i = 0;
            i = iActivityManager.startActivity(intent, info, resultTo, options, resultWho, requestCode, userId);
            return i;
        } catch (RemoteException e) {
            Log.d("kkkk", "==RemoteException==000==" + System.currentTimeMillis() / 1000);
            Integer crash = VirtualRuntime.crash(e);
            return crash;
        }
    }
    ...
    public int startActivity(Intent intent, int userId) {
        Log.d("kkkk", "==startActivity==666666666666666666==" + System.currentTimeMillis() / 1000);
        if (userId < 0) {
            Log.d("kkkk", "==userId==000==" + System.currentTimeMillis() / 1000);
        ...
        if (info == null) {
            Log.d("kkkk", "==info==000==" + System.currentTimeMillis() / 1000);
